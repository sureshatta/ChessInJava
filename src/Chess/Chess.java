...
/**
 * The Chess Board Game that will handle the input and maybe the state of the
 * game.
 *
 */
public class Chess {
	...
	/**
	 * The Constructor of the Chess game that creates the board and the players.
	 */
	public Chess() {
	...
	public void start() throws IOException {
		...
		while (true) {
			...
			if (i == 0) {
				...
				System.out
						.println("Example is a2 a3 (a2 is source, a3 is destination)");

			} else {
				System.out.println("*Black's turn.. *");
				System.out
						.println("Example is a7 a6 (a7 is source, a6 is destination)");
			}
			...
			if (success == 31 || success == 32 || success == 33
					|| success == 34) {
				try {
			...
			if (players[i].isInCheck()) {
				/*
				 * If player is in check (and we reach this if-statement, there
				 * IS a way for the king to get out of check redo the last move.
				 */
				king = (King) players[i].getKingRef();
				if (Check.scanCheck(king.getPosition().getRank(), king
						.getPosition().getFile(), king.getColor(), king.board)) {
					line = players[i].getLastMove();
					argv = line.split(" ");
				...
				/*
				 * Checks to make sure the last move did not indirectly cause
				 * the King to get into check by the player themselves.
				 */
				king = (King) players[i].getKingRef();
				if (Check.scanCheck(king.getPosition().getRank(), king
						.getPosition().getFile(), king.getColor(), king.board)) {
	...
	/**
	 * Translates a file-rank move to a move that would work with a 2D matrix.
	 * 
	 * @param move
	 *            The move to be translated.
	 ...
	 * @param end
	 *            The end location of the piece.
	 * @return 1 if the move was legal and no piece was taken, 2 if a peice was
	 *         taken and a bunch of other stuff based on
	 *         castling/pawnpromotion/enpassent
	 * @throws IllegalArgumentException
	 *             IllegalMove
	 */
	public int attemptMove(int[] start, int[] end, Player currP)
			throws IllegalArgumentException {
		...
		if (legal == 0 || legal == 3) {
			...
			board.getSquare(start[0], start[1]).removePiece();
			Piece removePiece = board.getSquare(end[0], end[1]).removePiece();
			addToRecentlyRem(removePiece);
			board.getSquare(end[0], end[1]).setPiece(toMove);
			/* Deals with promotion */
			if (end[0] == 7 && currP.getColor() == Color.White) {
				if (toMove instanceof Pawn) {
			...
			addToRecentlyRem(board.getSquare(end[0] + 1, end[1]).removePiece());
			board.getSquare(start[0], start[1]).removePiece();
			board.getSquare(end[0], end[1]).setPiece(toMove);
			return 21;
		} else if (legal == 22) {
	...
	private void addToRecentlyRem(Piece removePiece) {
		for (int i = 0; i < recentlyRem.length; i++) {
			if (recentlyRem[i] == null) {
				recentlyRem[i] = removePiece;
			}
	 ...
	 * 
	 * @param end
	 *            The square where the piece was moved to.
	 * @param start
	 *            The square where the piece should be placed again.
	 * @param currP
	 *            The current Player.
	 */
	public void redoLastMove(int[] end, int[] start, Player currP,
			int wasPieceCaptured) {
		...
		if (wasPieceCaptured == 2) {
			recRem = recentlyRemRemoveLastItem();
			...
			Piece p = recentlyRemRemoveLastItem();
	...
	private Piece recentlyRemRemoveLastItem() {
		Piece piece = null;
		for (int i = recentlyRem.length - 1; i < 0; i--) {
			if (recentlyRem[i] != null) {
				piece = recentlyRem[i];
				recentlyRem[i] = null;
			}
	 ...
	 * 
	 * @param end
	 *            The location of the pawn to be promoted
	 * @param argvlength
	 *            The length of the move to see if they specified a piece
	 * @param argv
	 *            The argument to tell what they want to promote the pawn to.
	 * @throws IllegalArgumentException
	 *             If the promotion is neither N,R,Q, or B.
	 */
	public void promotePawn(int[] end, int argvlength, String[] argv)
			throws IllegalArgumentException {
		...
		if (key != 0) {
			for (int i = 0; i < 8; i++) {
				if (safe[i] == 1) {
					switch (i) {
					case 0:
						if (Check.scanCheckKingSaved(Kx + 1, Ky - 1,
								p.getColor(), board))
							safe[i] = 0;
						break;
					case 1:
						if (Check.scanCheckKingSaved(Kx + 1, Ky, p.getColor(),
								board))
							safe[i] = 0;
						break;
					case 2:
						...
						break;
					case 3:
						if (Check.scanCheckKingSaved(Kx, Ky - 1, p.getColor(),
								board))
							safe[i] = 0;
						break;
					case 4:
			...
			for (int j = 0; j < 8; j++) {
				if (danger[j] != null) {
					danger[j] = Check
							.canBlock(danger[j].getPosition(),
									danger[j].getColor(), p.getPosition(),
									danger[j], j);
				}
			}
			...
			if (Knights.size() > 1)
				...
				if (Check.scanCheck(k.getPosition().getRank(), k.getPosition()
						.getFile(), k.getColor(), k.board)) {
					return 11;
				} else {
					return 12;
				}
	...
	/**
	 * Resets enPassent so pieces can not perform enPassent if they don't take
	 * advantange of it.
	 * 
	 * @param player
	 *            The index of the player to tell which row to clear.
	 */
